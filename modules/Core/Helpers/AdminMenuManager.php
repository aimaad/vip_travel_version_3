...
class AdminMenuManager
{
    ...
    public static function register_group($group, $name, $position = 10, $options = [])
    {
    ...
    public static function all()
    {
        ...
        $pendingAgentCount = \App\Models\RoleRequest::where('type', 'Agent de voyage')->where('status', 'pending')->count();
        $pendingDistributorCount = \App\Models\RoleRequest::where('type', 'Distributeur de voyage')->where('status', 'pending')->count();
        ...
        $menus = [
            ...
            'become_agent' => [
                "position" => 20,
                'url' => route('user.admin.agentUpgrade'),
                'title' => __("Agent Upgrade Requests") . ($pendingAgentCount > 0 ? ' <span class="badge badge-warning">' . $pendingAgentCount . '</span>' : ''),
            ...
            'become_distributor' => [
                "position" => 22,
                'url' => route('user.admin.distributorUpgrade'),
                'title' => __("Distributor Upgrade Requests") . ($pendingDistributorCount > 0 ? ' <span class="badge badge-warning">' . $pendingDistributorCount . '</span>' : ''),
                'icon' => 'fa fa-user-plus',
                'permission' => 'user_view',
                'group' => 'system' // This should match the group where other user-related items are.
...
if (!empty($custom_modules)) {
    ...
    foreach ($custom_modules as $moduleData) {
        ...
        if (class_exists($moduleClass)) {
            ...
            if (!empty($menuConfig)) {
                // Ensure the 'url' key exists before checking its value
                if (!isset($menuConfig['url']) || $menuConfig['url'] !== 'admin/module/tour/availability') {
        ...
        // Pro Menu
        $pro_modules = \Pro\ServiceProvider::getModules();
        if (!empty($pro_modules)) {
            foreach ($pro_modules as $module) {
                $moduleClass = "\\Pro\\" . ucfirst($module) . "\\ModuleProvider";
                if (class_exists($moduleClass)) {
                    $menuConfig = call_user_func([$moduleClass, 'getAdminMenu']);
