...
class UserController extends AdminController
{
    ...
    public function store(Request $request, $id)
    {
        ...
        if ($id && $id > 0) {
            ...
            if ($row->id != Auth::user()->id && !Auth::user()->hasPermission('user_update')) {
                abort(403);
            }
        } else {
        ...
        $rules = [
            'civilite'                => 'required|max:3',  // Add this line
        ...
        if ($role && ($role->name == 'Agent de voyage' || $role->name == 'Distributeur de voyage')) {
            $rules['agency_name'] = 'required|string|max:255';
            $rules['iata_office_id'] = 'nullable|string|max:255';
        }
    
        $request->validate($rules);
    
        $agency_name = $request->input('agency_name');
        $other_agency_name = $request->input('other_agency_name');
        if ($agency_name == 'other' && !empty($other_agency_name)) {
        ...
        $data = [
            ...
            'agency_name'              => $agency_name, // Sauvegarder le nom de l'agence (personnalisé ou sélectionné)
            'iata_office_id'           => $request->input('iata_office_id'),
        ];
        ...
        if ($row->save()) {
            return back()->with('success', ($id && $id > 0) ? __('User updated') : __("User created"));
        }
    }
