...
//                to admin
    ...
    public function hasPendingRoleRequest($roleName)
    {
        Log::info('Vérification de la demande en attente pour le rôle: ' . $roleName . ' pour l\'utilisateur ID: ' . $this->id);
    
        $exists = RoleRequest::where('user_id', $this->id)
            ->where('type', $roleName)
            ->where('status', 'pending')
            ->exists();
    
        Log::info('Résultat de la vérification: ' . ($exists ? 'Demande en attente trouvée' : 'Aucune demande en attente'));
